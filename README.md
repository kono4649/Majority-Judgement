...
| Auth | JWT (python-jose) ※投票作成・結果閲覧APIに認証必須。ユーザー登録は誰でも可能 |
| CI/CD | GitHub Actions |
| Proxy / Server | Nginx / Docker & Docker Compose |

## 📋 機能一覧

- **ユーザー登録**: 誰でも自由にアカウントを作成可能
...
| メソッド | パス | 認証 | 説明 |
|---|---|---|---|
| POST | `/api/v1/auth/register` | 不要 | ユーザー登録（誰でも可能） |
...
フロントエンドの登録画面からアカウントを作成してご利用ください。
...
| STEP | 内容 | テスト数 |
|------|------|--------|
| 1 | ユーザー登録（誰でも可能・重複拒否） | 3 |
| 2 | ログイン → 投票フォーム作成（未認証での作成拒否含む） | 4 |
| 3 | 不特定多数が投票（匿名・認証済み・重複拒否） | 5 |
| 4 | 作成者のみが投票結果を確認（非作成者・未認証は拒否） | 3 |

SQLiteインメモリDBを使用するため、外部DB不要で高速に実行できます。

## 🔄 CI/CD

GitHub Actions (`/.github/workflows/ci.yml`) により、`main` / `master` / `claude/**` ブランチへのプッシュ時（および `main` / `master` へのプルリクエスト時）に以下が自動実行されます：
